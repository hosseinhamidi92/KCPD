...
file_path = os.path.join(r'..\Irritation', Subject_ID + '.txt')
...
checkpoints_file = r'..\Checkpoints\Checkpoints_Irr.txt'
# Read the Checkpoints file
checkpoints = {}
...
input_file =  r"..\Time_Stamps\Experiment_1_Stamps_I.xlsx"
...
# Use the function to replace NaNs
...
result = algo_c.predict(pen=int(checkpoint_value))[:-1]  #n_bkps=8
...
for change_point in filtered_CP:
    ...
    directions = []
    for cp in result:  # Exclude the last point which is the signal length
...
for time_label, event_label in time_event_pairs:
    ...
    if event_label in ['Fog Start','Single Slow Car','Double Slow Car','Brake 1','Brake 2','Brake 3','Brake 4','Brake 5',]: ax1.axvspan(time_label - 5, time_label + window_size/2, color='green', alpha=0.1)  # Add shaded region ±1 second
    ...
    if event_label in ['Video Start', 'Survey 1 Start', 'Practice Start', 'Survey 2 Start',
        'Experiment Start', 'Fog Start', 'Brake 1','Crash','Barrel','Timer Start','Construction 1 Start',
        ...
        ax1.text(time_label, 0.7,  # Adjust vertical position as needed
                ...
                fontsize=8,
                color='green')
    elif event_label not in ['Stop 1','Stop 2','Stop 3','Stop 4','Timer Start','Pace Car Start','Pace Car Clear','Timer Stop','Amazon Truck','Clear Text']:
...
CLIP_VALUE = [int(video_start_time), int(video_end_time), int(practice_start_time), int(practice_end_time),
              int(experiment_start_time),int(experiment_end_time)]  # Update clip value to video end
...
for col in range(Used_signals_for_cpd.shape[1]):
    initial = Used_signals_for_cpd[clip_indices[0]:clip_indices[0]+100, col]
...
max_indices = np.argpartition(average_iks_statistics, -2)[-2:]
max_indices = max_indices[np.argsort(average_iks_statistics[max_indices])][::-1]

plt.plot(time_dummy[max_indices[0]], average_iks_statistics[max_indices[0]], 'ro', label='Max Point')
